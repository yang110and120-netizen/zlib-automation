...
改进版：使用系统 Chrome，参考 zlibrary-to-notebooklm 的实现
"""
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="从 Z-Library 下载书籍（测试版）")
    parser.add_argument("--url", default="https://zh.zlib.li/book/w9pzqBZP8b/%E9%95%BF%E5%AE%89%E7%9A%84%E8%8D%94%E6%9E%9D.html", help="Z-Library 书籍链接")
    parser.add_argument("--output", default=None, help="下载目录（默认：系统临时目录）")
    args = parser.parse_args()
    
    print("="*60)
    print("Z-Library 下载测试")
    print("="*60)
    ...
    print(f"[目录] 下载目录: {output_dir}")
    
    file_path = await download_book(args.url, output_dir)
